...
var Emap = new TextureLoader().load(T_earth, function (t) {
  var testCanvas = document.createElement('canvas'),
    ...
    Eh;
  var img = t.image;
  Ew = testCanvas.width = img.width;
  Eh = testCanvas.height = img.height;
  tCtx.scale(1, -1);
  tCtx.drawImage(img, 0, -Eh);
  var idata = tCtx.getImageData(0, 0, Ew, Eh);
  Egeometry.vertices.forEach((p, i) => {
    var u = 0.5 - Math.atan2(-p.z, -p.x) / 2 / PI,
      v = 0.5 + Math.asin(p.y / R) / PI,
    ...
    sh.fragmentShader = sh.fragmentShader.replace(
      ...
      `
		    ...
		    diffuseColor.a *= 1.-smoothstep(1. - delta, 1.+delta, r);
			  diffuseColor.a *= smoothstep( ${(R * 0.3).toFixed(1)},  ${(-R).toFixed(
          1
        )}, fogDepth-${posZ.toFixed(1)} );
			`
    );
    sh.vertexShader =
      `
...
Egeometry.vertices.forEach((v) => {
  Egeometry.uv.push(0.5 - Math.atan2(-v.z, -v.x) / 2 / PI);
  Egeometry.uv.push(0.5 + Math.asin(v.y / R) / PI);
});
...
' +
  ...
  float r = length(cxy), delta = fwidth(r), size=1.-vSize;
	...
	#endif
 diffuseColor.a *= smoothstep( ${(R * 0.2).toFixed(1)},  ${(-R * 0.4).toFixed(
   1
 )}, fogDepth-${posZ.toFixed(1)} )*size;
      `
      );
...
var t0 = performance.now(),
  ...
  af,
  Pactive = [],
  axis = vec3(0, 1, 0).applyAxisAngle(vec3(0, 0, 1), obliquity),
  points0 = [],
  pUp = 0,
  pDn = [],
  flTimer = [],
...
Tmaterial.size = 0.047 * d;
Tmaterial.opacity = 0.75;
Tmaterial.color.multiplyScalar(0.8);

function addTransaction(a, b, i) {
  //console.log (pUp, a, b); //return
  var an = a.angleTo(b),
    l = R * 1.13 + an * 5.5,
    center = a.clone().add(b).setLength(l),
    ab = b.clone().sub(a).multiplyScalar(0.25),
    cn = center.clone().setLength((l - R) * 0.7),
    n; //=an*160+16;
...
function addPoint(i0, i, c = 0) {
  ...
  if (!i) {
    if (!points[0]) i = 0;
      ...
      points.every(function (p, j) {
        return points[(i = j + 1 + '')];
      });
    if (i >= pCount && (!i0 || points[i0].isNew)) return false;
  }
  var fi = Math.random() * 1.8,
    dTest;
  var point = points0[Math.floor(Math.random() * points0.length)].clone();
  var dLast,
    pointW = Earth.localToWorld(point.clone()).applyAxisAngle(axis, roV1 * 150);
  ...
  if (
    points.some(
      (v, i) =>
        (v.up > -1 || flashes[i] > 0.05) && v.distanceTo(point) < R * (v.pInd == i0 ? 0.18 : 0.4)
    )
  )
    return addPoint(i0, i, c);
  point.pInd = i;
  points[i] = point;
  point.isNew = !i0;
  ...
  if (i0 && pUp < 6 && Math.random() > 0.65) {
    //fork
    flTimer[i0] = Math.random() < 0.5 ? Math.random() * 200 + 230 : Math.random() * 100;
    points[i0].up = 1;
  }
  return true;
}
...
window.addEventListener('pointermove', (e) => {
  ...
  dx = Math.lerp(dx, p.x0 - (p.x0 = e.clientX), 0.3);
  dy = Math.lerp(dy, p.y0 - (p.y0 = e.clientY), 0.3);
...
requestAnimationFrame(function animate() {
  ...
  dt = Math.min(dt, dMax);
  t0 = t;
  planet.position.z -= planet.position.z * 0.08;
  ...
  dx *= 1 - 0.0015 * dt;
  dy *= 1 - 0.0015 * dt;
  if (pointers.touch) document.scrollingElement.scrollTop += Math.round(dy * 0.3);
  planet.rotateOnWorldAxis(wX, -dy * 0.005);
  planet.rotateOnWorldAxis(wY, -dx * 0.005);
  var aCorr = Math.sqrt(1 - ax.angleTo(pAxis) / 3.15);
  planet.applyQuaternion(
    quat
      .clone()
      .setFromUnitVectors(ax, pAxis)
      .slerp(quat, 1 - 0.0008 * dt * aCorr)
  );
  ...
  points.forEach(function (p, i) {
    ...
    count++;
    if (p.up > 0) {
      if ((flashes[i] += (1.005 - flashes[i]) * 0.005 * dt) > 0.95) {
        p.up = -1;
      }
      if (!transactions[i] && ++pUp && flashes[i] > 0.15) newTr = i + '';
    }
    if (p.up < 0) {
      if ((flashes[i] -= (1.11 - flashes[i]) * flashes[i] * 0.006 * dt) < 0.005) {
        delete points[i];
      }
      if (flashes[i] < maxDn) newP = 1;
    }
    if (transactions[i]) {
      var arr = transactions[i].geometry.attributes.flash.array,
        n = arr.length,
        t = (transactions[i].timer += (dt / Math.pow(transactions[i].n, 0.3)) * 0.008); //, tt=t*t;
      arr.forEach(function (v, j) {
        var df = j / n - t,
          dj = n - j;
        arr[j] = df < 0 ? 1 + df : +(df < 0.2) * (1 - df * df * 8);
